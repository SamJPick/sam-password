...
OPTS=$(getopt -o hn:g:e:l --long help,new:,get:,exists:,list -n 'sam-password' -- $@)
if [ $# -lt 1 ]
then
	...
	while true; do
		case "$1" in
			...
			-g | --get)
				shift
				site=$1
				sam-password-encrypter g ' ' $site &> /dev/null
				if [ $? -eq 1 ]
				then
			...
			-e | --exists)
				shift
				site=$1
				sam-password-encrypter e $site &> /dev/null
				if [ $? -eq 1 ]
				then
